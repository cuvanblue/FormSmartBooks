...
abstract class BaseEloquentRepository
{
    ...
    public function all($columns = array('*'))
    {
        if ($this->model instanceof Builder) {
            $results = $this->model->orderBy('last_modified_date', 'ASC')->get($columns);
        } else {
            $results = $this->model->orderBy('last_modified_date', 'DESC')->get($columns);
        }
    ...
    public function joinOne($fieldjoin, $tablejoin, $columns = array('*'), $columjoin = array('*'))
    {
        ...
        $results = $this->model
            ->where("$tableName.last_modified_date", '=', null)
            ->leftjoin("$tablejoin as b", "b.id", '=', "$tableName.$fieldjoin")
            ->orderBy("$tableName.last_modified_date", 'DESC')
            ->get($select);
        $this->resetModel();
        return $results;
    }
